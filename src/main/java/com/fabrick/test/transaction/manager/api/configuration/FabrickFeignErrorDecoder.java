...
public class FabrickFeignErrorDecoder implements ErrorDecoder {
    ...
    public Exception decode(String methodKey, Response response) {
        ...
        return switch (status) {
            case BAD_REQUEST ->
                    new FabrickApiException(ErrorCode.EXTERNAL_API_FAILURE.getDefaultMessage(), status, fabrickStatus, ErrorCode.EXTERNAL_API_FAILURE);
            case UNAUTHORIZED ->
                    new FabrickApiException(ErrorCode.UNAUTHORIZED.getDefaultMessage(), status, fabrickStatus, ErrorCode.UNAUTHORIZED);
            case FORBIDDEN -> new FabrickApiForbiddenException(ErrorCode.FORBIDDEN.getDefaultMessage());
            case NOT_FOUND ->
                    new FabrickApiException(ErrorCode.NOT_FOUND.getDefaultMessage(), status, fabrickStatus, ErrorCode.NOT_FOUND);
            case INTERNAL_SERVER_ERROR ->
                    new FabrickApiException(ErrorCode.EXTERNAL_API_FAILURE.getDefaultMessage(), status, fabrickStatus, ErrorCode.EXTERNAL_API_FAILURE);
            case SERVICE_UNAVAILABLE ->
                    new FabrickApiException(ErrorCode.SERVICE_UNAVAILABLE.getDefaultMessage(), status, fabrickStatus, ErrorCode.SERVICE_UNAVAILABLE);
            default -> new Exception("Unexpected error occurred. Please contact support.");
        };
    }
